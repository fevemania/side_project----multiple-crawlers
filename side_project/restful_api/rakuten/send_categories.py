...
class RakutenProductCrawler:
    def __init__(self, downloader, fluentd_port=9880):
        ...
        self.channel.queue_declare(queue='rakuten_categories')
    ...
    def run(self):
        ...
        html = requests.post(self.search_page_url, json=search_page_payload)
        api_data = json.loads(html.text)
        categories = api_data['data']['searchPage']['baseFacetCategoryList']
        for category in categories:
            row = (category['id'], category['level'], category['isLeafNode'], int(math.ceil(category['count']/20)))
...
if __name__ == '__main__':
    ...
    try:
        ...
        rakuten_product_crawler.connection.close()
